import Link from "next/link";
import { notFound } from "next/navigation";
import { ArrowLeft, ArrowUp, ArrowDown, Boxes, List } from "lucide-react";
import { requireAuth, canManageWorkspace } from "@/lib/auth";
import { withTenant } from "@/lib/tenant";
import { deleteField, reorderField } from "@/lib/actions/objects";
import { listObjects, listFields } from "@/lib/objects-registry";
import { FIELD_TYPE_LABELS, relationTarget, isRelationType, type FieldType } from "@/lib/custom-objects";
import { PageHeader } from "@/components/app/page-header";
import { FieldForm } from "@/components/app/field-form";
import { ObjectPicker } from "@/components/app/object-picker";
import { DeleteButton } from "@/components/app/delete-button";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";

export const dynamic = "force-dynamic";

export default async function PropertiesSettingsPage({
  searchParams,
}: {
  searchParams: Promise<{ object?: string }>;
}) {
  const ctx = await requireAuth();
  if (!canManageWorkspace(ctx.role)) notFound();
  const sp = await searchParams;

  const data = await withTenant(ctx.workspaceId, async (tx) => {
    const objects = await listObjects(tx);
    const token = objects.find((o) => o.token === sp.object)?.token ?? "contact";
    const meta = objects.find((o) => o.token === token)!;
    const fields = await listFields(tx, token);
    const customTargets = await tx.objectDefinition.findMany({
      where: { slug: { not: token } },
      select: { slug: true, nameSingular: true },
    });
    return { objects, token, meta, fields, customTargets };
  });

  const { objects, token, meta, fields, customTargets } = data;

  return (
    <div className="max-w-3xl space-y-6">
      <PageHeader
        title="Properties"
        description="Properties store information on your records. Pick an object, then add the fields it should collect."
        action={
          <div className="flex items-center gap-2">
            <Link href="/app/settings">
              <Button variant="ghost" className="gap-2">
                <ArrowLeft className="h-4 w-4" /> Settings
              </Button>
            </Link>
            <Link href="/app/settings/properties/all">
              <Button variant="outline" className="gap-2">
                <List className="h-4 w-4" /> All properties
              </Button>
            </Link>
            <Link href="/app/objects">
              <Button variant="outline" className="gap-2">
                <Boxes className="h-4 w-4" /> Objects
              </Button>
            </Link>
          </div>
        }
      />

      <Card>
        <CardContent className="flex flex-wrap items-center gap-3 py-4">
          <span className="text-sm font-medium text-muted-foreground">Select an object:</span>
          <ObjectPicker objects={objects.map((o) => ({ token: o.token, label: o.pluralLabel }))} value={token} />
          <Badge variant="secondary" className="ml-auto">{meta.kind === "core" ? "Built-in" : "Custom"} object</Badge>
        </CardContent>
      </Card>

      <Card>
        <CardHeader>
          <CardTitle className="text-base">{meta.label} properties ({fields.length})</CardTitle>
        </CardHeader>
        <CardContent>
          {fields.length === 0 ? (
            <p className="text-sm text-muted-foreground">No custom properties yet. Add one below.</p>
          ) : (
            <ul className="divide-y divide-border rounded-md border border-border">
              {fields.map((f, i) => {
                const rel = isRelationType(f.type) ? relationTarget(f.options) : null;
                return (
                  <li key={f.id} className="flex items-center justify-between gap-2 px-3 py-2">
                    <div className="min-w-0">
                      <div className="truncate text-sm font-medium">
                        {f.label}
                        {f.required && <span className="ml-1 text-destructive">*</span>}
                      </div>
                      <div className="mt-0.5 text-xs text-muted-foreground">
                        <Badge variant="secondary">{FIELD_TYPE_LABELS[f.type as FieldType] ?? f.type}</Badge>
                        {rel && <span className="ml-1">→ {rel}</span>}
                      </div>
                    </div>
                    <div className="flex shrink-0 items-center gap-1">
                      <form action={reorderField.bind(null, f.id, token, "up")}>
                        <button type="submit" disabled={i === 0} className="rounded p-1 text-muted-foreground hover:bg-accent disabled:opacity-30" aria-label="Move up">
                          <ArrowUp className="h-4 w-4" />
                        </button>
                      </form>
                      <form action={reorderField.bind(null, f.id, token, "down")}>
                        <button type="submit" disabled={i === fields.length - 1} className="rounded p-1 text-muted-foreground hover:bg-accent disabled:opacity-30" aria-label="Move down">
                          <ArrowDown className="h-4 w-4" />
                        </button>
                      </form>
                      <DeleteButton action={deleteField.bind(null, f.id, token)} label="" confirmText={`Delete property "${f.label}"?`} />
                    </div>
                  </li>
                );
              })}
            </ul>
          )}
        </CardContent>
      </Card>

      <Card>
        <CardHeader>
          <CardTitle className="text-base">Create property</CardTitle>
        </CardHeader>
        <CardContent>
          <FieldForm token={token} customTargets={customTargets} />
        </CardContent>
      </Card>
    </div>
  );
}
