...
- **Retrieval + grounding upgrade (no migration, no creds)** — shipped the highest-leverage autonomous items from the recovered research. `src/lib/knowledge.ts`: over-fetch a wide candidate pool (≤60) then a free **lexical reranker** (exact-term overlap + saturating density + whole-phrase hit + title hit, blended 0.6 SQL / 0.4 lexical) trims to top-k; SQL ranking moved `ts_rank`→**`ts_rank_cd`** (cover density); passages reordered for **lost-in-the-middle** (strongest first, 2nd-strongest last) and **numbered `[n]`** for citation. `src/lib/agent-presets.ts` + `knowledge.ts buildSystemPrompt`: grounding block strengthened to the conservative spec — answer ONLY from `<knowledge>`, **cite source number(s)**, explicit "say you don't have that information" abstention, no outside knowledge. Still lexical-only (embeddings/pgvector hybrid remains the credential-gated next step). Pure-function ordering verified; `tsc` + `next build` green.
